...
if ( ! class_exists( 'RWMB_Image_Field' ) )
{
	class RWMB_Image_Field extends RWMB_File_Field
	{
		...
		static function wp_ajax_reorder_images()
		{
			...
			RW_Meta_Box::ajax_response( __( 'Order saved', 'locality' ), 'success' );
		}
		...
		{
			$i18n_title = _x( 'Upload images', 'image upload', 'locality' );
			$i18n_more  = _x( '+ Add new image', 'image upload', 'locality' );
		...
		{
			$i18n_delete = _x( 'Delete', 'image upload', 'locality' );
			$i18n_edit   = _x( 'Edit', 'image upload', 'locality' );
			$li = '
				<li id="item_%s">
					<img src="%s" />
